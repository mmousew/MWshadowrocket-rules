...
async function addMissingRuleSetColumns() {
  ...
  if (!columns.has("platform_sources")) statements.push(db.prepare("ALTER TABLE rule_sets ADD COLUMN platform_sources text DEFAULT '{}' NOT NULL"));
  if (statements.length) await db.batch(statements);
}
...
async function initializeDatabaseSchema() {
  ...
  await db.batch([
    ...
    db.prepare("CREATE INDEX IF NOT EXISTS clash_profile_source_selections_profile_idx ON clash_profile_source_selections (profile_id)"),
    db.prepare("CREATE INDEX IF NOT EXISTS clash_profile_source_selections_source_idx ON clash_profile_source_selections (source_id)"),
