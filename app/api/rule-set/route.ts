...
function clientRows(rows: Awaited<ReturnType<typeof listRuleSets>>, usages: RuleSetUsageRow[] = []) {
  const usageMap = new Map<string, RuleSetUsageRow[]>();
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ruleSets: clientRows(ruleSets, await listRuleSetUsages()), bindings });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ruleSets: clientRows(await listRuleSets(), await listRuleSetUsages()) });
  } catch (error) {
