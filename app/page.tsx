    ...
    <main className="shell">
      ...
      <section className="content">
        ...
        {(view === "groups" || view === "rules" || view === "sets") && <>
          ...
          {view === "groups" ? <><p className={`dragHelp ${query ? "disabled" : ""}`}>{query ? "清空搜索后可以拖拽调整完整分组顺序" : "按住左侧或整行拖动排序；也可以用上移/下移"}</p>{parsed.finalRule && <section className="finalRuleCard"><div className="finalRuleCardMain"><span className="finalRuleType">FINAL</span><div><strong>最终兜底规则</strong><p>未匹配到其它规则的流量 → {parsed.finalRule.policy}</p><code>FINAL,{parsed.finalRule.policy}</code></div></div><button type="button" className="primary" onClick={() => { const finalGroup = parsed.groups.find((group) => group.name === parsed.finalRule?.policy); if (finalGroup) editGroup(finalGroup); else setError(`兜底规则引用的分组「${parsed.finalRule?.policy}」不存在`); }}>配置 FINAL 节点</button></section>}<div className="listPanel">{filteredGroups.map((group) => { const linked = parsed.rules.filter((rule) => rule.policy === group.name); const isFinalGroup = parsed.finalRule?.policy === group.name; const linkedCount = linked.length + (isFinalGroup ? 1 : 0); return <div
