...
type View = "overview" | "groups" | "rules" | "sets" | "rule-library" | "configs" | "clash" | "airports" | "conflicts";
...
function normalizeGroupVisibility(records: unknown): Record<string, boolean> {
  const result: Record<string, boolean> = {};
  if (!Array.isArray(records)) return result;
  for (const item of records as Array<{ group_name?: string; groupName?: string; visible?: number | boolean }>) {
    const name = String(item.groupName ?? item.group_name ?? "").trim();
    if (!name) continue;
    result[policyKey(name)] = item.visible !== false && item.visible !== 0;
  }
  return result;
}

function mergeGroupNames(...lists: string[][]) {
  const result: string[] = [];
...
function groupSourceLabel(name: string, countryGroups: string[]) {
  if (["proxies", "final", "cn"].includes(policyKey(name))) return "系统";
  if (countryGroups.some((item) => policyKey(item) === policyKey(name))) return "国家";
  return "自定义";
}
...
function readGroupFilter(raw: string, countryGroups: string[], existingGroups: string[] = []) {
  ...
  const allGroups = mergeGroupNames(countryGroups, existingGroups);
  const countryKeys = new Map(countryGroups.map((item) => [policyKey(item), item]));
  const groupKeys = new Map(allGroups.map((item) => [policyKey(item), item]));
...
function updateGroupItems(raw: string, countryGroups: string[], existingGroups: string[], changes: { kind?: string; country?: string; group?: string; keyword?: string; includeAll?: boolean }) {
  const current = readGroupFilter(raw, countryGroups, existingGroups);
  const kind = changes.kind || current.kind;
  const groups = new Set(current.selectedGroups);
  const allGroups = mergeGroupNames(countryGroups, existingGroups);
  const toggleGroup = (value: string) => {
    const canonical = allGroups.find((item) => policyKey(item) === policyKey(value)) || value.trim();
    if (!canonical) return;
    if (groups.has(canonical)) groups.delete(canonical);
    else groups.add(canonical);
  };
  if (changes.country) toggleGroup(changes.country);
  if (changes.group) toggleGroup(changes.group);
  const isKnownGroup = (item: string) => allGroups.some((value) => policyKey(value) === policyKey(item));
  let extras = current.values.slice(1).filter((item) => !isKnownGroup(item) && !/^policy-regex-filter=/i.test(item) && policyKey(item) !== "include-all-proxies=true");
  ...
  return [kind, ...includeItems, ...Array.from(groups), ...keywordItems, ...extras].join("\n");
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([configResponse, ruleConfigResponse]) => {
        ...
        setSourceUrl(data.sourceUrl); setSaveEnabled(data.saveEnabled); setRuleConfigs(configs); setSelectedRuleConfigId(selected?.id || "default"); setGroupVisibility(normalizeGroupVisibility(ruleConfigData.groupVisibility)); setLoading(false);
      })
  ...
  useEffect(() => {
    ...
    fetch(`/api/rule-config?id=${encodeURIComponent(selectedRuleConfigId)}`, { cache: "no-store" })
      .then(async (response) => {
        const data = await response.json();
        if (response.status === 401) { setAuthRequired(true); return; }
        if (!response.ok) throw new Error(data.error || "读取分组显示设置失败");
        setGroupVisibility(normalizeGroupVisibility(data.groupVisibility));
      })
      .catch((cause) => setError(cause instanceof Error ? cause.message : "读取分组显示设置失败"));
  }, [selectedRuleConfigId]);

  useEffect(() => {
    if (!selectedRuleConfigId) return;
    fetch(`/api/rule-set?config=${encodeURIComponent(selectedRuleConfigId)}`, { cache: "no-store" })
      .then(async (response) => {
        const data = await response.json();
        if (response.status === 401) { setAuthRequired(true); return; }
  ...
  const activeRules = viewingGroup ? parsed.rules.filter((rule) => rule.policy === viewingGroup) : view === "sets" ? inlineRuleSets : domainRules;
  ...
  async function toggleGroupVisibility(group: Group, visible: boolean) {
    const previous = groupVisibility;
    const next = { ...groupVisibility, [policyKey(group.name)]: visible };
    const settings = parsed.groups.map((item) => ({
      groupName: item.name,
      visible: next[policyKey(item.name)] !== false,
    }));
    setGroupVisibility(next);
    setGroupVisibilityBusy(true);
    setError("");
    try {
      const response = await fetch("/api/rule-config", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: selectedRuleConfigId, groupVisibility: settings }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "更新客户端分组显示状态失败");
      setGroupVisibility(normalizeGroupVisibility(data.groupVisibility));
      setToast(`客户端中的「${group.name}」已${visible ? "显示" : "隐藏"}`);
    } catch (cause) {
      setGroupVisibility(previous);
      setError(cause instanceof Error ? cause.message : "更新客户端分组显示状态失败");
    } finally {
      setGroupVisibilityBusy(false);
    }
  }
  ...
  function submitEditor(event: FormEvent) {
    ...
    if (editor.mode === "group") {
      ...
      if (editor.index === null) {
        ...
        const nextBindings = { ...ruleSetBindings };
        const oldBindingKey = oldGroup ? policyKey(oldGroup.name) : "";
        const currentRuleSetIds = oldBindingKey ? (ruleSetBindings[oldBindingKey] || []) : [];
        if (oldBindingKey) delete nextBindings[oldBindingKey];
        if (currentRuleSetIds.length) nextBindings[policyKey(name)] = currentRuleSetIds;
        setRuleSetBindings(nextBindings);
        markContent(next);
      }
    } else {
  ...
  function confirmDelete() {
    ...
    if (deleteTarget.kind === "group") {
      const nextBindings = { ...ruleSetBindings };
      delete nextBindings[policyKey(deleteTarget.group.name)];
      setRuleSetBindings(nextBindings);
    }
    setDeleteTarget(null);
  }
  ...
  return (
    <main className="shell">
      ...
      <section className="content">
        ...
        {view === "rule-library" && <RuleSetLibrary ruleSets={availableRuleSets} loading={ruleSetsLoading} onChange={setAvailableRuleSets} onToast={setToast} onError={setError} />}
      ...
      {groupRulesModal && <GroupRulesModal configId={selectedRuleConfigId} group={groupRulesModal} ruleSets={(ruleSetBindings[policyKey(groupRulesModal.name)] || []).map((ruleSetId) => availableRuleSets.find((ruleSet) => ruleSet.id === ruleSetId)).filter(Boolean) as RuleSetRecord[]} availableRuleSets={availableRuleSets} selectedRuleSetIds={ruleSetBindings[policyKey(groupRulesModal.name)] || []} onRuleSetChange={(ruleSetIds) => { const groupKey = policyKey(groupRulesModal.name); setRuleSetBindings((current) => { const next = { ...current }; if (ruleSetIds.length) next[groupKey] = ruleSetIds; else delete next[groupKey]; return next; }); setDirty(true); setToast(`「${groupRulesModal.name}」的规则集调用已更新，请保存方案`); }} tempRules={activeTempRulesByGroup[policyKey(groupRulesModal.name)] || []} onTempRulesChange={(rules) => { setTempRulesConfigId(selectedRuleConfigId); setTempRulesByGroup((current) => ({ ...current, [policyKey(groupRulesModal.name)]: rules })); }} query={groupRulesQuery} onQueryChange={setGroupRulesQuery} onClose={() => setGroupRulesModal(null)} />}
...
function RuleConfigManager({ configs, selectedId, busy, onEdit, onCreate, onRename, onDelete }: {
  ...
  return <section className="panel ruleConfigPanel">
    ...
    <div className="ruleConfigList">{configs.map((config) => <article className={`ruleConfigCard ${config.id === selectedId ? "selected" : ""}`} key={config.id}>
      ...
      <div className="ruleConfigActions">{config.id === "default" && <span className="configTemplateBadge">复制默认</span>}<button type="button" className="ghost" disabled={busy} onClick={() => onEdit(config.id)}>编辑方案</button>{config.id !== "default" && <button type="button" className="danger" disabled={busy} onClick={() => onDelete(config.id)}>删除</button>}</div>
    </article>)}</div>
...
function ClashSubscription({ mode = "private" }: { mode?: "private" | "airports" }) {
  ...
  const profileLinkCard = (link: LinkRecord) => <article className={`clashLinkCard ${link.status === "revoked" ? "revoked" : ""}`} key={link.id}>
    ...
    <details className="legacySubscriptionLinks"><summary>兼容旧链接（可选）</summary><label className="clientLinkLabel">CLASH 地址<div className="clientLinkRow"><input readOnly value={clashRelayUrl(link.url, link.name)} onFocus={(event) => event.currentTarget.select()} /><button type="button" className="inlineCopy" onClick={() => void copyLink(clashRelayUrl(link.url, link.name))}>复制</button>{link.status === "active" && <button type="button" className="inlineQr" onClick={() => void showQr(clashRelayUrl(link.url, link.name), "CLASH")}>二维码</button>}</div></label><label className="clientLinkLabel">小火箭订阅地址<div className="clientLinkRow"><input readOnly value={shadowrocketUrl(link.url, link.name)} onFocus={(event) => event.currentTarget.select()} /><button type="button" className="inlineCopy" onClick={() => void copyLink(shadowrocketUrl(link.url, link.name))}>复制订阅</button>{link.status === "active" && <a className="inlineImport" href={shadowrocketAddUrl(link.url, link.name)}>添加订阅</a>}{link.status === "active" && <button type="button" className="inlineQr" onClick={() => void showQr(shadowrocketAddUrl(link.url, link.name), "小火箭订阅")}>二维码</button>}</div></label></details>
    <label className="clientLinkLabel">小火箭配置地址（仅规则与分组）<div className="clientLinkRow"><input readOnly value={shadowrocketConfigUrl(link.url, link.name)} onFocus={(event) => event.currentTarget.select()} /><button type="button" className="inlineCopy" onClick={() => void copyLink(shadowrocketConfigUrl(link.url, link.name))}>复制配置</button>{link.status === "active" && <a className="inlineImport" href={shadowrocketImportUrl(link.url, link.name)}>导入配置</a>}{link.status === "active" && <button type="button" className="inlineQr" onClick={() => void showQr(shadowrocketImportUrl(link.url, link.name), "小火箭配置")}>二维码</button>}</div></label>
  </article>;
...
function GroupRulesModal({ configId, group, ruleSets, availableRuleSets, selectedRuleSetIds, onRuleSetChange, tempRules, onTempRulesChange, query, onQueryChange, onClose }: { configId: string; group: Group; ruleSets: RuleSetRecord[]; availableRuleSets: RuleSetRecord[]; selectedRuleSetIds: string[]; onRuleSetChange: (value: string[]) => void; tempRules: TempRuleRecord[]; onTempRulesChange: (rules: TempRuleRecord[]) => void; query: string; onQueryChange: (value: string) => void; onClose: () => void }) {
  ...
  return <div className="modalBackdrop" role="button" tabIndex={0} aria-label={`关闭${group.name}规则`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} onKeyDown={(event) => { if (event.key === "Escape") onClose(); }}>
    <section className="groupRulesModal" role="dialog" aria-modal="true" aria-labelledby="group-rules-title">
      ...
      <RuleSetSelect title="调用规则集" description="可多选；这里的选择只影响当前方案的这个分组。规则集会按选择顺序合并并自动去重。" value={selectedRuleSetIds} ruleSets={availableRuleSets} onChange={onRuleSetChange} />
      <div className="groupRulesSummary"><span><strong>{ruleSets.reduce((count, ruleSet) => count + ruleSet.entryCount, 0)}</strong> 条规则{tempRules.length ? ` · ${tempRules.length} 条临时` : ""}</span>{ruleSets.length ? <span className={ruleSetDisabled ? "groupRulesState disabled" : "groupRulesState"}>当前调用：{ruleSets.map((ruleSet) => ruleSet.name).join("、")}{ruleSetDisabled ? " · 含已停用规则集" : ""}</span> : <span className="groupRulesState">未调用规则集</span>}</div>
...
function EditorModalLegacy({ editor, setEditor, policies, countryGroups, existingGroups, ruleSets, error, onSubmit, onImportCatalog }: { editor: Editor; setEditor: (value: Editor | null) => void; policies: string[]; countryGroups: string[]; existingGroups: string[]; ruleSets: RuleSetRecord[]; error: string; onSubmit: (event: FormEvent) => void; onImportCatalog: (items: CatalogResult[], policy: string) => void }) {
  const [catalogQuery, setCatalogQuery] = useState("");
  const [catalog, setCatalog] = useState<CatalogResult[]>([]);
  const [catalogLoading, setCatalogLoading] = useState(false);
  const [catalogError, setCatalogError] = useState("");

  const selectableExistingGroups = mergeGroupNames(countryGroups, existingGroups).filter((group) => editor.mode !== "group" || policyKey(group) !== policyKey(editor.name));
  function updateGroupConfig(changes: { country?: string; group?: string; keyword?: string; includeAll?: boolean }) {
    if (editor.mode !== "group") return;
    setEditor({ ...editor, items: updateGroupItems(editor.items, [], selectableExistingGroups, changes) });
  }
...
type EditorModalProps = { editor: Editor; setEditor: (value: Editor | null) => void; policies: string[]; countryGroups: string[]; existingGroups: string[]; ruleSets: RuleSetRecord[]; error: string; onSubmit: (event: FormEvent) => void; onImportCatalog: (items: CatalogResult[], policy: string) => void };
...
function GroupNodeFilter({ raw, countryGroups, existingGroups, onChange }: { raw: string; countryGroups: string[]; existingGroups: string[]; onChange: (changes: { country?: string; group?: string; keyword?: string; includeAll?: boolean }) => void }) {
  ...
  return <section className="friendlyGroupConfig"><div className="groupSectionHeading"><strong>节点来源</strong><small>系统、国家和自定义分组统一选择；“全部节点”和“关键词筛选”只能二选一。</small></div>{allGroups.length > 0 ? <><strong className="nodeSourceHeading">添加已有分组</strong><div className="countryChecks existingGroupChecks">{allGroups.map((group) => <label key={group}><input type="checkbox" checked={config.selectedGroups.has(group)} onChange={() => onChange({ group })} /><span>{group}</span><small className="groupSourceTag">{groupSourceLabel(group, countryGroups)}</small></label>)}</div></> : null}<div className="nodeSourceMode" role="radiogroup" aria-label="节点来源方式"><label className="friendlyOption"><input type="radio" name={sourceId} checked={config.includeAll} onChange={() => onChange({ includeAll: true })} />选择全部节点</label><label className="friendlyOption"><input type="radio" name={sourceId} checked={!config.includeAll} onChange={() => onChange({ includeAll: false })} />关键词筛选</label></div><label>节点关键词 <small>用英文竖线 | 分隔，例如：YouTube|Google|美国；选择“关键词筛选”后填写。</small><input value={config.keyword} disabled={config.includeAll} onChange={(event) => onChange({ keyword: event.target.value })} placeholder="例如：YouTube|youtube|YT" /></label></section>;
}

function RuleSetSelect({ value, ruleSets, onChange, title = "选择规则集", description }: { value: string[]; ruleSets: RuleSetRecord[]; onChange: (value: string[]) => void; title?: string; description?: string }) {
  ...
  return <section className="ruleSetSelect"><div className="ruleSetSelectHeading"><strong>{title} <small>可多选</small></strong>{selected.length ? <button type="button" className="ghost" onClick={() => onChange([])}>清空</button> : null}</div><div className="ruleSetMultiList">{activeSets.map((ruleSet) => <label key={ruleSet.id} className="ruleSetMultiOption"><input type="checkbox" checked={selected.includes(ruleSet.id)} onChange={() => toggle(ruleSet.id)} /><span><strong>{ruleSet.name}</strong><small>{ruleSet.entryCount} 条</small></span></label>)}</div><small>{selected.length ? (description || "按选择顺序合并，重复规则自动去重。") : (description || "不使用规则集（只作为分组）")}</small></section>;
}

function NewGroupEditor({ editor, setEditor, countryGroups, existingGroups, ruleSets, error, onSubmit }: { editor: NewGroupEditor; setEditor: (value: Editor | null) => void; countryGroups: string[]; existingGroups: string[]; ruleSets: RuleSetRecord[]; error: string; onSubmit: (event: FormEvent) => void }) {
  const regularKinds = editor.regularKinds || [];
  const regularItems = editor.regularItems || {};
  const regularRuleSets = editor.regularRuleSets || {};
  const customEnabled = Boolean(editor.customEnabled);
  const customItems = editor.customItems || defaultGroupItems("select");
  const allAvailableGroups = mergeGroupNames(countryGroups, existingGroups);
  const customName = (editor.customName || "").trim();
  const selectableCustomGroups = allAvailableGroups.filter((group) => !customName || policyKey(group) !== policyKey(customName));
  const selectableCustomCountries = countryGroups.filter((group) => selectableCustomGroups.some((item) => policyKey(item) === policyKey(group)));
  const selectableCustomExistingGroups = existingGroups.filter((group) => selectableCustomGroups.some((item) => policyKey(item) === policyKey(group)));
  ...
  function toggleRegular(kind: string) {
    ...
    setEditor({ ...editor, regularKinds: nextKinds, regularItems: nextItems, regularRuleSets });
  }
  ...
  function updateRegular(kind: string, changes: { country?: string; group?: string; keyword?: string; includeAll?: boolean }) {
    ...
    setEditor({ ...editor, regularItems: { ...regularItems, [kind]: updateGroupItems(raw, countryGroups, existingGroups, changes) }, regularRuleSets });
  }

  function updateCustom(changes: { country?: string; group?: string; keyword?: string; includeAll?: boolean }) {
    setEditor({ ...editor, customItems: updateGroupItems(customItems, selectableCustomCountries, selectableCustomExistingGroups, changes) });
  }
  ...
  return <div className="modalBackdrop" role="button" tabIndex={0} aria-label="关闭编辑窗口" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditor(null); }} onKeyDown={(event) => { if (event.key === "Escape") setEditor(null); }}><form className="editorModal" aria-label="新增代理分组" onSubmit={onSubmit}><header><div><h2>新增代理分组</h2><p>常规分组和自定义分组分别保存，互不影响。</p></div><button type="button" onClick={() => setEditor(null)}>×</button></header>{error && <div className="editorError" role="alert"><span>!</span><pre>{error}</pre></div>}
    ...
    <section className="customGroupConfig"><div className="groupSectionHeading"><strong>自定义分组</strong><small>与上面的常规分组独立；可以无限新增，但名称仍需唯一，避免规则无法判断。</small></div><label className="friendlyOption customEnableOption"><input type="checkbox" checked={customEnabled} onChange={(event) => setEditor({ ...editor, customEnabled: event.target.checked })} />启用自定义分组</label>{customEnabled && <><label>分组名称<input value={editor.customName || ""} onChange={(event) => setEditor({ ...editor, customName: event.target.value })} placeholder="例如：YouTube" /></label><GroupNodeFilter raw={customItems} countryGroups={selectableCustomCountries} existingGroups={selectableCustomExistingGroups} onChange={updateCustom} /></>}</section>
...
function EditorModal(props: EditorModalProps) {
  ...
  return isNewGroup ? <NewGroupEditor editor={props.editor} setEditor={props.setEditor} countryGroups={props.countryGroups} existingGroups={props.existingGroups} ruleSets={props.ruleSets} error={props.error} onSubmit={props.onSubmit} /> : <EditorModalLegacy {...props} />;
}
...
function RuleSetLibrary({ ruleSets, loading, onChange, onToast, onError }: { ruleSets: RuleSetRecord[]; loading: boolean; onChange: (value: RuleSetRecord[]) => void; onToast: (value: string) => void; onError: (value: string) => void }) {
  ...
  async function saveRuleSet(event: FormEvent) {
    event.preventDefault();
    if (!name.trim()) return onError("规则集名称不能为空");
    ...
    } catch (cause) { onError(cause instanceof Error ? cause.message : "保存规则集失败"); }
    finally { setSaving(false); }
  }
  ...
  async function removeRuleSet(ruleSet: RuleSetRecord) {
    ...
    setSaving(true); onError("");
    try {
      const response = await fetch("/api/rule-set", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: ruleSet.id }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "删除规则集失败");
      onChange((data.ruleSets || []) as RuleSetRecord[]); if (editingId === ruleSet.id) closeEditor(); onToast(`规则集「${ruleSet.name}」已删除`);
    } catch (cause) { onError(cause instanceof Error ? cause.message : "删除规则集失败"); }
    finally { setSaving(false); }
  }
