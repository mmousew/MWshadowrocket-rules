...
function ClashSubscription() {
  ...
  async function generateLink(event: FormEvent) {
    ...
    setGenerating(true); setError(""); setWarning(""); setGeneratedNodes(null);
    try {
      ...
      setLinks((current) => [data.link, ...current]); setGeneratedNodes(data.nodeCount); setCopied(false); setSourceUrl(""); if (data.warning) setWarning(data.warning);
