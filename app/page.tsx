...
function ClashSubscription() {
  ...
  async function generateLink(event: FormEvent) {
    ...
    try {
      ...
      form.set("sourceUrls", sourceUrls.join("\n"));
      sourceFiles.forEach((file) => form.append("airportFiles", file, file.name));
      const response = await fetch("/api/clash/link", { method: "POST", body: form });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "生成订阅失败");
      setLinks((current) => [data.link, ...current]); setGeneratedNodes(data.nodeCount); setCopied(false); setSourceUrl(""); setSourceFiles([]); if (data.warning) setWarning(data.warning);
